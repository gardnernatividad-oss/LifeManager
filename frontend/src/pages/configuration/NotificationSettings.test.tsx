...
describe("NotificationSettings", () => {
  ...
  it("shows unsupported and retries the same canonical preferences loader", async () => { Object.defineProperty(globalThis, "Notification", { configurable: true, value: undefined }); vi.mocked(api.getNotificationPreferences).mockRejectedValueOnce(new Error("private")).mockResolvedValueOnce(preferences); const user = userEvent.setup(); mount(); expect(await screen.findByRole("alert")).toHaveTextContent("No pudimos cargar"); await user.click(screen.getByRole("button", { name: "Reintentar" })); expect(await screen.findByRole("heading", { name: "Notificaciones" })).toBeInTheDocument(); expect(await screen.findByRole("button", { name: "Guardar notificaciones" })).toBeInTheDocument(); expect(api.getNotificationPreferences).toHaveBeenCalledTimes(2); });
});
