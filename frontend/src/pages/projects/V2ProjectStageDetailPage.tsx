...
import { correctV2ProjectStageProgress, getV2ProjectStage, listV2ProjectStageHistory, updateV2ProjectStageProgress } from "../../api/v2ProjectStageApi";
...
function WorkspaceStageDetail({ workspaceId, projectId, stageId, mode }: { workspaceId: string; projectId: string; stageId: string; mode: "planning" | "tracking" }) {
  const client = useQueryClient(); const [progress, setProgress] = useState(""); const [comment, setComment] = useState(""); const [feedback, setFeedback] = useState<string | null>(null); const [correcting, setCorrecting] = useState(false);
  ...
  const mutation = useMutation({ mutationFn: () => correcting ? correctV2ProjectStageProgress(workspaceId, projectId, stageId, { progress, ...(comment.trim() ? { comment: comment.trim() } : {}), lock_version: stage!.lock_version, project_lock_version: project.data!.lock_version }) : updateV2ProjectStageProgress(workspaceId, projectId, stageId, { ...(progress === "" ? {} : { progress }), ...(comment.trim() ? { comment: comment.trim() } : {}), lock_version: stage!.lock_version, project_lock_version: project.data!.lock_version }), onSuccess: async () => { setProgress(""); setComment(""); setCorrecting(false); setFeedback(correcting ? "Corrección guardada." : "Seguimiento guardado."); await Promise.all([client.invalidateQueries({ queryKey: queryKeys.v2ProjectStages(workspaceId, projectId) }), client.invalidateQueries({ queryKey: queryKeys.v2ProjectStageDetail(workspaceId, projectId, stageId) }), client.invalidateQueries({ queryKey: queryKeys.v2ProjectStageHistory(workspaceId, projectId, stageId) }), client.invalidateQueries({ queryKey: queryKeys.v2ProjectDetail(workspaceId, projectId) }), client.invalidateQueries({ queryKey: queryKeys.v2ProjectsRoot(workspaceId) })]); }, onError: async (error) => { setFeedback(axios.isAxiosError(error) && error.response?.status === 409 ? "La Etapa o el Proyecto cambiaron. Actualizamos los datos." : "No pudimos guardar el seguimiento."); setCorrecting(false); await Promise.all([stageQuery.refetch(), project.refetch(), history.refetch()]); } });
  ...
  const canCorrect = mode === "planning" && stage.can_correct_progress;
  return <section className="project-stage-detail-page"><Link className="back-link" to={`${basePath}/${projectId}`}>← Volver al Proyecto</Link><header><p className="eyebrow">{project.data.name} · Etapa</p><h1>{stage.name}</h1></header>
    ...
    <section className="stage-tracking"><h2>Comentarios / Seguimiento</h2>{feedback ? <p role="status">{feedback}</p> : null}{stage.can_update_progress || correcting ? <form onSubmit={submit}><label>{correcting ? "Avance corregido (%)" : "Nuevo avance (%)"}<input required={correcting} type="number" min="0" max={correcting ? "99.99" : "100"} step="0.01" value={progress} onChange={(event) => setProgress(event.target.value)} placeholder={stage.progress} /></label><label>Comentario<textarea maxLength={2000} value={comment} onChange={(event) => setComment(event.target.value)} /></label><button disabled={mutation.isPending}>{correcting ? "Guardar corrección" : "Guardar seguimiento"}</button>{correcting ? <button type="button" onClick={() => { setCorrecting(false); setProgress(""); }}>Cancelar</button> : null}</form> : <><p>{project.data.is_active ? "La Etapa finalizada es de solo lectura." : "Reactiva el Proyecto para guardar seguimiento."}</p>{canCorrect ? <button type="button" onClick={() => { setCorrecting(true); setProgress(""); }}>Corregir avance</button> : null}</>}</section>
    <section className="pending-history"><h2>Historial</h2>{history.isPending ? <p role="status">Cargando historial…</p> : history.isError ? <div role="alert"><p>No pudimos cargar el historial.</p><button type="button" onClick={() => void history.refetch()}>Reintentar</button></div> : history.data.items.length === 0 ? <p>Sin registros todavía.</p> : <ol>{history.data.items.map((entry) => <li key={entry.id}><div><strong>{labels[entry.type]}</strong><time dateTime={entry.recorded_at}>{new Intl.DateTimeFormat("es-PE", { dateStyle: "short", timeStyle: "short" }).format(new Date(entry.recorded_at))}</time></div><p>{entry.actor_display_name} · {entry.previous_progress !== null ? `${entry.previous_progress}% → ` : ""}{entry.progress}%</p>{entry.comment ? <p className="pending-history-comment">{entry.comment}</p> : null}</li>)}</ol>}</section>
  </section>;
}
