...
describe("V2 Project hierarchical detail", () => {
  ...
  it("supports comment-only", async () => { const user = userEvent.setup(); mount("/seguimiento/proyectos/project-1/etapas/stage-1", <V2ProjectStageDetailPage mode="tracking" />); await screen.findByRole("heading", { name: "Empacar" }); await user.type(screen.getByLabelText("Comentario"), "Sin cambio"); await user.click(screen.getByRole("button", { name: "Guardar seguimiento" })); await waitFor(() => expect(stageApi.updateV2ProjectStageProgress).toHaveBeenCalledWith("workspace-a", "project-1", "stage-1", { comment: "Sin cambio", lock_version: 2, project_lock_version: 4 })); });

  it("clears unsent Stage state and scopes queries when Workspace changes", async () => { const user = userEvent.setup(); const view = <V2ProjectStageDetailPage mode="tracking" />; const mounted = mount("/seguimiento/proyectos/project-1/etapas/stage-1", view); await screen.findByRole("heading", { name: "Empacar" }); await user.type(screen.getByLabelText("Comentario"), "Privado"); auth.workspace.id = "workspace-b"; mounted.rerender(<MemoryRouter initialEntries={["/seguimiento/proyectos/project-1/etapas/stage-1"]}><QueryClientProvider client={new QueryClient({ defaultOptions: { queries: { retry: false } } })}><Routes><Route path="/seguimiento/proyectos/:projectId/etapas/:stageId" element={view} /></Routes></QueryClientProvider></MemoryRouter>); await waitFor(() => expect(stageApi.getV2ProjectStage).toHaveBeenLastCalledWith("workspace-b", "project-1", "stage-1")); expect(await screen.findByLabelText("Comentario")).toHaveValue(""); });

  it("renders a finalized Stage read-only", async () => { vi.mocked(stageApi.getV2ProjectStage).mockResolvedValue({ ...stage, progress: 100, state: "FINALIZADA", completion_date: "2026-09-10", can_edit: false, can_update_progress: false }); mount("/seguimiento/proyectos/project-1/etapas/stage-1", <V2ProjectStageDetailPage mode="tracking" />); expect(await screen.findByText("La Etapa finalizada es de solo lectura.")).toBeInTheDocument(); expect(screen.queryByRole("button", { name: "Guardar seguimiento" })).not.toBeInTheDocument(); });
});
