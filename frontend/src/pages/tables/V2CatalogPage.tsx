...
function WorkspaceCatalogPage({ kind, label, singular, workspace }: WorkspaceProps) {
  ...
  const update = useMutation({ mutationFn: () => {
    ...
    const categoryChanged = !isCategory && (editing as V2CatalogItem).category_id !== (original as V2CatalogItem | undefined)?.category_id;
    return updateV2Catalog(workspaceId, kind, editing!.id, { name: editing!.name, ...(categoryChanged ? { category_id: (editing as V2CatalogItem).category_id } : {}), lock_version: editing!.lock_version });
  }, onSuccess: async () => { setEditing(null); setMessage(`${singular} actualizada.`); await refresh(); }, onError: async (error) => { setEditing(null); setMessage(safeMessage(error)); await refresh(); } });
