...
import { createV2Catalog, listV2Catalog, setV2CatalogActive, updateV2Catalog, type CatalogKind, type V2CatalogItem, type V2Category } from "../../api/v2CatalogApi";
import { queryKeys } from "../../api/queryKeys";
import { useAuth } from "../../hooks/useAuth";

interface Props { kind: CatalogKind; label: "Categorías" | "Tareas" | "Actividades"; singular: "Categoría" | "Tarea" | "Actividad"; }
...
export function V2CatalogPage({ kind, label, singular }: Props) {
  ...
  const key = queryKeys.v2Catalog(workspaceId, kind, params);
  const query = useQuery({ queryKey: key, queryFn: () => listV2Catalog<V2Category>(workspaceId, kind, params), enabled: Boolean(workspaceId) });
  ...
  const refresh = () => client.invalidateQueries({ queryKey: queryKeys.v2CatalogRoot(workspaceId) });
