...
describe("V2 HomePage", () => {
  ...
  it("renders four clickable today cards and the global sections", async () => { mount(); expect(await screen.findByRole("heading", { name: "Inicio" })).toBeInTheDocument(); expect(screen.getByRole("link", { name: /Tareas\s*1/ })).toHaveAttribute("href", "/revision"); expect(screen.getByRole("link", { name: /Pendientes\s*2/ })).toBeInTheDocument(); expect(screen.getByRole("link", { name: /Etapas\s*3/ })).toBeInTheDocument(); expect(screen.getByRole("link", { name: /Actividades\s*4/ })).toBeInTheDocument(); expect(screen.getByRole("heading", { name: "Próximas Actividades" })).toBeInTheDocument(); expect(screen.getByRole("heading", { name: "Requieren atención" })).toBeInTheDocument(); expect(screen.getByRole("heading", { name: "Próximos días" })).toBeInTheDocument(); });
  it("renders activity, safe Review navigation, workspace indicators and seven days", async () => { mount(); expect(await screen.findByRole("link", { name: /Consulta/ })).toHaveTextContent("Personal"); expect(screen.getByRole("link", { name: /Mudanza · Empacar/ })).toHaveAttribute("href", "/revision"); expect(screen.getAllByRole("heading", { level: 3 })).toHaveLength(7); });
  ...
  it("keeps zero-count cards navigable", async () => { vi.mocked(homeApi.getV2HomeSummary).mockResolvedValue({ ...summary, today: { tasks: 0, pending_items: 0, project_stages: 0, activities: 0 } }); mount(); expect(await screen.findByRole("link", { name: /Tareas\s*0/ })).toHaveAttribute("href", "/revision"); expect(screen.getByRole("link", { name: /Pendientes\s*0/ })).toBeInTheDocument(); expect(screen.getByRole("link", { name: /Etapas\s*0/ })).toBeInTheDocument(); expect(screen.getByRole("link", { name: /Actividades\s*0/ })).toBeInTheDocument(); });
