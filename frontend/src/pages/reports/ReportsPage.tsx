...
function ActivityBreakdown({ title, rows }: { title: string; rows: ActivityReportGroup[] }) {
  const dimension = title.replace("Por ", "");
  return <><h3>{title}</h3><div className="report-table-wrap"><table aria-label={title}><thead><tr><th>{dimension}</th><th>Cantidad</th><th>Duración total</th><th>Duración promedio</th></tr></thead><tbody>{rows.map((row) => <tr key={row.key}><td data-label={dimension}>{row.label}</td><td data-label="Cantidad">{row.total_count}</td><td data-label="Duración total">{row.total_duration_minutes} min</td><td data-label="Duración promedio">{row.average_duration_minutes === null ? "—" : `${row.average_duration_minutes} min`}</td></tr>)}</tbody></table></div></>;
}
...
function ProgressBreakdowns({ categories, evolution }: { categories: ProgressCategoryGroup[]; evolution: ProgressEvolution[] }) {
  return <><h3>Por Categoría</h3><div className="report-table-wrap"><table aria-label="Distribución por Categoría"><thead><tr><th>Categoría</th><th>Total</th><th>Avance promedio</th></tr></thead><tbody>{categories.map((row) => <tr key={row.category_id}><td data-label="Categoría">{row.category_name}</td><td data-label="Total">{row.total_count}</td><td data-label="Avance promedio">{row.average_progress === null ? "—" : `${row.average_progress}%`}</td></tr>)}</tbody></table></div><h3>Distribución por fecha planificada</h3><div className="report-table-wrap"><table aria-label="Distribución por fecha planificada"><thead><tr><th>Fecha</th><th>Total</th><th>Avance promedio</th></tr></thead><tbody>{evolution.map((row) => <tr key={row.planned_date}><td data-label="Fecha">{formatShortCalendarDate(row.planned_date)}</td><td data-label="Total">{row.total_count}</td><td data-label="Avance promedio">{row.average_progress === null ? "—" : `${row.average_progress}%`}</td></tr>)}</tbody></table></div></>;
}

function TaskBreakdown({ title, rows }: { title: string; rows: TaskReportGroup[] }) {
  const dimension = title === "Por Tarea" ? "Tarea" : "Categoría";
  return <><h3>{title}</h3><div className="report-table-wrap"><table aria-label={title}><thead><tr><th>{dimension}</th><th>Total</th><th>Completadas</th><th>No realizadas</th><th>Cumplimiento</th></tr></thead><tbody>{rows.map((row) => <tr key={row.key}><td data-label={dimension}>{row.label}</td><td data-label="Total">{row.total_count}</td><td data-label="Completadas">{row.completed_count}</td><td data-label="No realizadas">{row.not_completed_count}</td><td data-label="Cumplimiento">{row.completion_rate === null ? "—" : `${row.completion_rate}%`}</td></tr>)}</tbody></table></div></>;
}

function TaskEvolutionTable({ rows }: { rows: TaskReportEvolution[] }) {
  return <><h3>Distribución por fecha planificada</h3><div className="report-table-wrap"><table aria-label="Distribución de Tareas por fecha planificada"><thead><tr><th>Fecha</th><th>Pendientes</th><th>Completadas</th><th>No realizadas</th></tr></thead><tbody>{rows.map((row) => <tr key={row.planned_date}><td data-label="Fecha">{formatShortCalendarDate(row.planned_date)}</td><td data-label="Pendientes">{row.pending_count}</td><td data-label="Completadas">{row.completed_count}</td><td data-label="No realizadas">{row.not_completed_count}</td></tr>)}</tbody></table></div></>;
}
