...
vi.mock("../../api/v2CalendarComparisonApi", () => ({ getCalendarComparisonMulti: vi.fn() }));
...
describe("CalendarComparisonPage", () => {
  ...
  async function selectTarget(user: ReturnType<typeof userEvent.setup>) {
    ...
    await waitFor(() => expect(screen.getByRole("checkbox", { name: "Luis" })).toBeInTheDocument());
    expect(screen.queryByRole("checkbox", { name: "Fuera" })).not.toBeInTheDocument();
    await user.click(screen.getByRole("checkbox", { name: "Luis" }));
  }
  it("renders DETAILS read-only with back navigation and no target actions", async () => {
    vi.mocked(comparisonApi.getCalendarComparisonMulti).mockResolvedValue({ members: [{ user_id: "target", display_name: "Luis", calendar: { visibility: "SHOW_DETAILS", detailed_events: [{ activity_name: "Médico", starts_at: "2027-01-01T15:00:00Z", ends_at: "2027-01-01T16:00:00Z", temporal_state: "FUTURE" }] } }] });
  ...
  it("renders only opaque busy blocks and handles HIDE as privacy state", async () => {
    vi.mocked(comparisonApi.getCalendarComparisonMulti).mockResolvedValueOnce({ members: [{ user_id: "target", display_name: "Luis", calendar: { visibility: "AVAILABILITY_ONLY", busy_blocks: [{ starts_at: "2027-01-01T15:00:00Z", ends_at: "2027-01-01T17:00:00Z", occupied: true }] } }] });
    ...
    unmount(); vi.mocked(comparisonApi.getCalendarComparisonMulti).mockResolvedValue({ members: [{ user_id: "target", display_name: "Luis", calendar: { visibility: "HIDE" } }] }); mount(); await selectTarget(userEvent.setup());
