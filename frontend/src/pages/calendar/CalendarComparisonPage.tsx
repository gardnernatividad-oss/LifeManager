...
import { getCalendarComparisonMulti } from "../../api/v2CalendarComparisonApi";
...
export function CalendarComparisonPage() {
  ...
  const comparison = useQuery({
    queryKey: queryKeys.calendarComparison(user?.id ?? "anonymous", workspaceId, targetIds.join(","), range?.from ?? "", range?.to ?? ""),
    queryFn: () => getCalendarComparisonMulti(workspaceId, targetIds, range!.from, range!.to),
    enabled: Boolean(user && workspaceId && targetIds.length && range),
  });
  ...
  return <section className="calendar-comparison-page">
    ...
    <div className="calendar-comparison-controls">
      <label>Workspace compartido<select aria-label="Workspace compartido" value={workspaceId} onChange={(event) => { setWorkspaceId(event.target.value); setTargetIds([]); client.removeQueries({ queryKey: ["calendar-comparison"] }); }}><option value="">Selecciona</option>{shared.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
      <fieldset><legend>Miembros</legend>{targets.map((item) => <label key={item.user_id}><input type="checkbox" checked={targetIds.includes(item.user_id)} disabled={!workspaceId || members.isPending} onChange={(event) => setTargetIds((current) => event.target.checked ? [...current, item.user_id] : current.filter((id) => id !== item.user_id))} />{item.display_name}</label>)}</fieldset>
    ...
    <div className="calendar-comparison-columns">
      ...
      <section><h3>Calendarios comparados</h3>{!targetIds.length ? <p className="review-empty">Selecciona uno o más miembros.</p> : comparison.isPending ? <p role="status">Cargando comparación…</p> : comparison.isError ? <div role="alert"><p>No pudimos cargar la comparación.</p><button onClick={() => void comparison.refetch()}>Reintentar</button></div> : comparison.data.members.map((member) => <div className="comparison-member" key={member.user_id}><h4>{member.display_name}</h4>{member.calendar.visibility === "HIDE" ? <p className="review-empty">Este usuario no comparte su calendario.</p> : member.calendar.visibility === "AVAILABILITY_ONLY" ? member.calendar.busy_blocks.length ? member.calendar.busy_blocks.map((block) => <article className="comparison-event comparison-event--busy" key={`${block.starts_at}-${block.ends_at}`}><strong>Ocupado</strong><span>{time(block.starts_at, user.timezone)}–{time(block.ends_at, user.timezone)}</span></article>) : <p className="review-empty">Sin bloques ocupados.</p> : member.calendar.detailed_events.length ? member.calendar.detailed_events.map((event) => <article className="comparison-event comparison-event--target" key={`${event.starts_at}-${event.ends_at}-${event.activity_name}`}><strong>{event.activity_name}</strong><span>{time(event.starts_at, user.timezone)}–{time(event.ends_at, user.timezone)}</span></article>) : <p className="review-empty">Sin actividades compartidas.</p>}</div>)}</section>
    </div>
  </section>;
}
