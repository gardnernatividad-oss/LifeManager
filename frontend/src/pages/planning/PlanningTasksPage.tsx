...
import { createV2Task, deleteV2Task, listV2Tasks, resolveV2Task, updateV2Task } from "../../api/v2TaskApi";
...
import type { V2Task, V2TaskFilters, V2TaskResult, V2TaskState } from "../../types/v2Task";
...
function WorkspaceTasks({ workspace }: { workspace: WorkspaceSummary }) {
  ...
  const create = useMutation({ mutationFn: () => createV2Task(workspaceId, { master_task_id: masterTaskId, planned_date: plannedDate, ...(shared ? { responsible_user_id: responsibleUserId } : {}) }), onSuccess: async () => { setMasterTaskId(""); setPlannedDate(""); setResponsibleUserId(""); setError(null); setNotice("Tarea creada."); await refresh(); }, onError: (caught) => { setNotice(null); setError(messageFor(caught)); } });
  ...
  return <section className="planning-tasks-page">
    ...
    <section className="planning-create" aria-labelledby="create-v2-task"><h2 id="create-v2-task">Crear tarea</h2><form onSubmit={(event) => { event.preventDefault(); if (!masterTaskId || !plannedDate || (shared && !responsibleUserId)) { setError("Completa los campos requeridos."); return; } create.mutate(); }}>
      <TaskCatalogSelector workspaceId={workspaceId} value={masterTaskId} onChange={setMasterTaskId} required />
      <label>Fecha<input type="date" value={plannedDate} onChange={(event) => setPlannedDate(event.target.value)} required /></label>
