...
type Editing = { task: V2Task; masterTaskId: string; plannedDate: string; responsibleUserId: string; scope: V2TaskMutationScope };
...
function WorkspaceTasks({ workspace }: { workspace: WorkspaceSummary }) {
  ...
  const beginEdit = (task: V2Task, scope: V2TaskMutationScope) => { setScopeAction(null); setEditing({ task, masterTaskId: task.master_task_id, plannedDate: task.planned_date, responsibleUserId: task.responsible_user_id, scope }); };
  ...
  return <section className="planning-tasks-page">
    ...
    <section className="planning-register" aria-labelledby="v2-task-list"><h2 id="v2-task-list">Tareas planificadas</h2><div className="planning-filters">
      ...
      <TaskCatalogSelector workspaceId={workspaceId} value={filters.master_task_id ?? ""} onChange={(value) => setFilters({ ...filters, page: 1, master_task_id: value || undefined })} />
