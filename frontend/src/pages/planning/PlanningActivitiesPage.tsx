...
function EditDialog({ activity, workspace, timeZone, members, pending, onClose, onSave }: { activity: V2Activity; workspace: WorkspaceSummary; timeZone: string; members: WorkspaceMemberSummary[]; pending: boolean; onClose: () => void; onSave: (payload: V2ActivityUpdate) => void }) { const shared = workspace.kind === "SHARED"; const [scope, setScope] = useState<ActivityMutationScope>("THIS"); const [masterId, setMasterId] = useState(activity.activity_master_id ?? ""); const [organizerId, setOrganizerId] = useState(activity.organizer_user_id); const [selected, setSelected] = useState(activity.participants.map((item) => item.user_id)); const [error, setError] = useState<string | null>(null); return <dialog open><form onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); try { onSave({ activity_master_id: masterId, ...(shared ? { organizer_user_id: organizerId, participant_user_ids: selected } : {}), starts_at: localDateTimeToIso(String(data.get("starts_at")), timeZone), ends_at: localDateTimeToIso(String(data.get("ends_at")), timeZone), lock_version: activity.lock_version, scope }); } catch (caught) { setError(caught instanceof Error ? caught.message : "Fecha u hora inválida."); } }}><h2>Editar {activity.title}</h2>{activity.is_generated ? <fieldset><legend>Aplicar a</legend><label><input type="radio" checked={scope === "THIS"} onChange={() => setScope("THIS")} />Solo esta</label><label><input type="radio" checked={scope === "THIS_AND_FUTURE"} onChange={() => setScope("THIS_AND_FUTURE")} />Esta y todas las futuras</label></fieldset> : null}<ActivityCatalogSelector workspaceId={workspace.id} currentId={activity.activity_master_id ?? undefined} value={masterId} onChange={setMasterId} required />{shared ? <MemberSelect label="Organizador" value={organizerId} members={members} onChange={setOrganizerId} required /> : null}<label>Inicio<input name="starts_at" type="datetime-local" required defaultValue={isoToLocalInput(activity.starts_at, timeZone)} /></label><label>Fin<input name="ends_at" type="datetime-local" required defaultValue={isoToLocalInput(activity.ends_at, timeZone)} /></label>{shared ? <ParticipantChecks members={members} selected={selected} onChange={setSelected} /> : null}{error ? <p role="alert">{error}</p> : null}<button disabled={pending}>Guardar</button><button type="button" onClick={onClose}>Cancelar</button></form></dialog>; }
