...
function WorkspaceProjects({ workspace }: { workspace: WorkspaceSummary }) {
  ...
  return <section className="v2-project-page"><header><p className="eyebrow">Planificación</p><h1>Planificación · Proyectos</h1></header>
    ...
    {detail.data ? <section className="project-planning-panel"><h2>{detail.data.name}</h2><p>{detail.data.description || "Sin descripción"}</p><p>{detail.data.category_name} · Líder: {detail.data.leader_display_name}</p><p>{detail.data.weights_complete ? `Avance ${detail.data.progress}% · ${detail.data.state} · ${detail.data.compliance ?? "Sin cumplimiento"}` : detail.data.stage_count === 0 ? "Sin Etapas · Avance 0% · No iniciado" : `Configuración incompleta (${detail.data.total_weight}% de 100%)`}</p><ProjectStagesPanel workspaceId={workspaceId} project={detail.data} shared={shared} members={activeMembers} /><button type="button" onClick={() => setSelectedId(null)}>Cerrar</button></section> : null}
...
function ProjectRow({ project, pending, onView, onEdit, onLifecycle }: { project: V2Project; pending: boolean; onView: () => void; onEdit: () => void; onLifecycle: () => void }) { return <article className="v2-project-row"><div><strong>{project.name}</strong><small>{project.category_name} · {project.leader_display_name}</small></div><span>{project.is_active ? "Activo" : "Inactivo"}</span><span>{project.weights_complete ? `Avance: ${project.progress}%` : project.stage_count === 0 ? "Avance: 0%" : "Pesos incompletos"}</span><div><button type="button" onClick={onView}>Ver</button>{project.can_edit ? <button type="button" onClick={onEdit}>Editar</button> : null}<button type="button" disabled={pending} onClick={onLifecycle}>{project.is_active ? "Desactivar" : "Reactivar"}</button></div></article>; }
