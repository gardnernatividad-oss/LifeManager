...
import { createV2Project, deactivateV2Project, listV2Projects, reactivateV2Project, updateV2Project } from "../../api/v2ProjectApi";
...
function WorkspaceProjects({ workspace }: { workspace: WorkspaceSummary }) {
  ...
  const mutation = useMutation({ mutationFn: (operation: () => Promise<V2Project>) => operation(), onSuccess: async (saved) => { setEditing(null); setFeedback("Cambios guardados."); await refresh(saved.id); }, onError: async (error) => { setEditing(null); setFeedback(safeError(error)); await refresh(); } });

  function submitCreate(event: FormEvent) {
    event.preventDefault();
    if (!categoryId || !name.trim() || (shared && !leaderId)) { setFeedback("Completa los campos requeridos."); return; }
    mutation.mutate(() => createV2Project(workspaceId, { category_id: categoryId, ...(shared ? { leader_user_id: leaderId } : {}), name: name.trim(), description: description.trim() || null }).then((created) => { setName(""); setDescription(""); return created; }));
  }
...
function ProjectRow({ project, pending, onEdit, onLifecycle }: { project: V2Project; pending: boolean; onEdit: () => void; onLifecycle: () => void }) { return <article className="v2-project-row"><div><strong>{project.name}</strong><small>{project.category_name} · {project.leader_display_name}</small></div><span>{project.is_active ? "Activo" : "Inactivo"}</span><span>{project.weights_complete ? `Avance: ${project.progress}%` : project.stage_count === 0 ? "Avance: 0%" : "Pesos incompletos"}</span><div><Link aria-label={`Abrir Proyecto ${project.name}`} to={`/planificacion/proyectos/${project.id}`}>›</Link>{project.can_edit ? <button type="button" onClick={onEdit}>Editar</button> : null}<button type="button" disabled={pending} onClick={onLifecycle}>{project.is_active ? "Desactivar" : "Reactivar"}</button></div></article>; }
