...
vi.mock("../../components/common/V2CatalogSelector", () => ({ ActivityCatalogSelector: ({ value, onChange, includeCustomOption }: { value: string; onChange: (value: string) => void; includeCustomOption?: boolean }) => <label>Actividad<select value={value} onChange={(event) => onChange(event.target.value)}><option value="">Selecciona</option><option value="master-1">Reunión</option>{includeCustomOption ? <option value="__custom__">Otra actividad...</option> : null}</select></label>, CategorySelector: ({ value, onChange }: { value: string; onChange: (value: string) => void }) => <label>Categoría<select aria-label="Categoría" value={value} onChange={(event) => onChange(event.target.value)}><option value="">Selecciona</option><option value="category-1">Familia</option></select></label> }));
...
describe("PlanningActivitiesPage", () => {
  ...
  it("creates a Shared Activity with organizer, participants and timezone-aware instants", async () => {
    ...
    await user.type(within(form).getByLabelText("Inicio"), "2027-01-01T10:00"); await user.type(within(form).getByLabelText("Fin"), "2027-01-01T11:00"); await user.type(within(form).getByLabelText("Recordatorio (minutos antes)"), "30"); await user.click(within(form).getByLabelText("Luis Dos")); await user.click(within(form).getByRole("button", { name: "Crear" }));
    await waitFor(() => expect(api.createV2Activity).toHaveBeenCalledWith("workspace-a", expect.objectContaining({ activity_master_id: "master-1", organizer_user_id: "user-1", participant_user_ids: ["user-2"], reminder_minutes_before: 30, starts_at: "2027-01-01T15:00:00.000Z", ends_at: "2027-01-01T16:00:00.000Z" })));
  });
  ...
  it("creates a recurring Otra actividad with its real name and manual Category", async () => {
    const user = userEvent.setup(); mount(); const form = screen.getByRole("heading", { name: "Crear Actividad" }).closest("section")!;
    await user.click(within(form).getByLabelText("Repetir")); await user.selectOptions(within(form).getByLabelText("Actividad"), "__custom__"); await user.type(within(form).getByLabelText("Nombre"), "Visita familiar"); await user.selectOptions(within(form).getByLabelText("Categoría"), "category-1"); await user.selectOptions(within(form).getByLabelText("Organizador"), "user-1"); await user.type(within(form).getByLabelText("Desde"), "2027-01-01"); await user.type(within(form).getByLabelText("Hasta"), "2027-01-01"); await user.type(within(form).getByLabelText("Hora de inicio"), "09:00"); await user.type(within(form).getByLabelText("Hora de fin"), "10:00"); await user.click(within(form).getByRole("button", { name: "Crear" }));
    await waitFor(() => expect(api.createRecurringV2Activities).toHaveBeenCalledWith("workspace-a", expect.objectContaining({ custom_name: "Visita familiar", custom_category_id: "category-1" })));
  });
