...
import type { V2RecurringTaskCreate, V2RecurringTaskCreateResponse, V2Task, V2TaskCreate, V2TaskFilters, V2TaskList, V2TaskResult, V2TaskUpdate } from "../types/v2Task";
...
export async function deleteV2Task(workspaceId: string, taskId: string, lockVersion: number): Promise<void> { await apiClient.delete(url(workspaceId, `/${taskId}`), { params: { lock_version: lockVersion } }); }
