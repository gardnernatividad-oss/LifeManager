...
export async function correctV2TaskResult(workspaceId: string, taskId: string, result: V2TaskResult, lockVersion: number): Promise<V2Task> { return (await apiClient.post<V2Task>(url(workspaceId, `/${taskId}/correct-result`), { lock_version: lockVersion, result })).data; }
