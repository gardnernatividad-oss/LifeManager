...
describe("planningTaskApi", () => {
  ...
  it("loads every MasterTask page in batches of 100 without Workspace parameters", async () => {
    ...
    expect(apiClient.get).toHaveBeenNthCalledWith(1, "http://localhost:3000/api/v1/master-tasks", { params: { page: 1, page_size: 100 } });
    expect(apiClient.get).toHaveBeenNthCalledWith(2, "http://localhost:3000/api/v1/master-tasks", { params: { page: 2, page_size: 100 } });
  });
  it("uses exact target Task endpoints and payloads", async () => {
    ...
    expect(apiClient.post).toHaveBeenCalledWith("http://localhost:3000/api/v1/tasks", { master_task_id: "master", planned_date: "2026-08-20" });
    ...
    expect(apiClient.post).toHaveBeenCalledWith("http://localhost:3000/api/v1/tasks/bulk", expect.objectContaining({ weekdays: [0, 2] }));
    await api.updatePlanningTask("task", { planned_date: "2026-08-21", lock_version: 3 });
    expect(apiClient.patch).toHaveBeenCalledWith("http://localhost:3000/api/v1/tasks/task", { planned_date: "2026-08-21", lock_version: 3 });
    await api.deletePlanningTask("task", 3);
    expect(apiClient.delete).toHaveBeenCalledWith("http://localhost:3000/api/v1/tasks/task", { params: { lock_version: 3 } });
    await api.deletePlanningTasksBulk([{ id: "task", lock_version: 3 }]);
    expect(apiClient.post).toHaveBeenCalledWith("http://localhost:3000/api/v1/tasks/bulk-delete", { items: [{ id: "task", lock_version: 3 }] });
  });
});
