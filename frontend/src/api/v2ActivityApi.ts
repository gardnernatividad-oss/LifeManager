...
export async function createRecurringV2Activities(workspaceId: string, payload: V2RecurringActivityCreate): Promise<V2RecurringActivityCreateResponse> {
  return (await apiClient.post<V2RecurringActivityCreateResponse>(url(workspaceId, "/recurring"), payload)).data;
}
