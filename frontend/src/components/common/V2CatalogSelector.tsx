...
export function V2CatalogSelector({ workspaceId, kind, label, value, onChange, currentId, required, includeCustomOption }: Props) {
  ...
  if (query.data.length === 0 && !includeCustomOption) return <p>No hay opciones activas disponibles.</p>;
  return <label>{label}<select aria-label={label} required={required} value={value} onChange={(event) => onChange(event.target.value)}><option value="">Selecciona una opción</option>{query.data.map((option) => <option key={option.id} value={option.id}>{option.name}{!option.is_active ? " (Inactiva)" : ""}</option>)}{includeCustomOption ? <option value="__custom__">Otra {label.toLowerCase()}...</option> : null}</select></label>;
}
