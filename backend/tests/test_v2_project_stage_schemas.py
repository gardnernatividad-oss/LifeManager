...
from app.schemas.v2_project_stage import ProjectStageCreate, ProjectStageProgress, ProjectStageUpdate
...
def test_stage_schemas_are_strict_and_weighted_projection_is_derived() -> None:
    created = ProjectStageCreate(name="  Preparar   cajas ", responsible_user_id=uuid.uuid4(), position=0, weight="40.00", planned_date=date(2026, 9, 10), project_lock_version=1)
    assert created.name == "Preparar cajas" and created.weight == Decimal("40.00")
    for payload in ({"name": "X", "position": 0, "weight": 0, "planned_date": "2026-09-10", "project_lock_version": 1}, {"name": "X", "position": 0, "weight": 40, "planned_date": "2026-09-10", "project_lock_version": 1, "workspace_id": str(uuid.uuid4())}):
    ...
    assert complete["progress"] == Decimal("80") and complete["state"] == "EN_PROCESO"
