...
V2 inicial no añade refresh token ni sesión persistida porque el modelo aprobado no contiene sesión/revocación. La cookie expira como máximo en una ventana corta configurable; logout y cambio de contraseña eliminan la cookie del dispositivo actual. Las cookies ya emitidas en otros dispositivos permanecen válidas solo hasta esa expiración corta, salvo que la cuenta pase a DISABLED. Si se exige revocación instantánea por dispositivo o sesiones largas, deberá diseñarse una entidad Session en una ADR/model migration posterior, no simularse con localStorage.
